...
import { getProductById } from "./mock-producten";
...
function regel(productId: string, aantal: number): BestellingRegel {
  const p = getProductById(productId);
  if (!p) throw new Error("Product " + productId + " niet gevonden");
  return {
    productId: p.id,
    ean: p.ean ?? p.sku,
    sku: p.sku,
    naam: p.naam,
    aantal,
    eenheidsprijs: p.prijs,
    gepicked: false,
  };
}

const PRODUCTEN_PER_MERK: Record<string, string[]> = {
  "leather-design": ["ld-crossbody", "ld-schoudertas", "ld-shoulder-natural", "ld-boodschappen", "ld-doctors-bag"],
  "orange-fire": ["of-ratpack-messenger", "ld-crossbody", "ld-schoudertas"],
  "ratpack": ["of-ratpack-messenger", "ld-crossbody"],
  "gaz": ["ld-boodschappen", "ld-ritstasje"],
  "shelby-brothers": ["ld-dames-hand-schouder", "ld-shoulder-backpack", "ld-doctors-bag"],
};

function buildRegels(merkId: string, totaal: number): BestellingRegel[] {
  const ids = PRODUCTEN_PER_MERK[merkId] ?? PRODUCTEN_PER_MERK["leather-design"];
  const regels: BestellingRegel[] = [];
  let rest = totaal;
  for (let i = 0; i < ids.length && rest > 0; i++) {
    const p = getProductById(ids[i]);
    if (!p) continue;
    const aantal = i === 0 ? Math.max(1, Math.floor(rest / p.prijs)) : 1;
    regels.push(regel(ids[i], aantal));
    rest -= aantal * p.prijs;
  }
  if (regels.length === 0 && ids[0]) {
    try {
      regels.push(regel(ids[0], 1));
    } catch (_) {}
  }
  return regels;
}

const BASE: Array<{ id: string; merkId: MerkId; ordernummer: string; klantNaam: string; klantEmail: string; totaal: number; status: BestellingStatus; datum: string }> = [
  ...
  { id: "2", merkId: "leather-design", ordernummer: "LD-2024-042", klantNaam: "Maria van Berg", klantEmail: "maria@example.nl", totaal: 245.00, status: "te_plukken", datum: "2024-02-25T09:15:00Z" },
  ...
  { id: "5", merkId: "shelby-brothers", ordernummer: "SB-2024-027", klantNaam: "Tom Smit", klantEmail: "tom@example.nl", totaal: 312.00, status: "te_plukken", datum: "2024-02-25T08:00:00Z" },
  ...
  { id: "12", merkId: "shelby-brothers", ordernummer: "SB-2026-015", klantNaam: "Daan Mulder", klantEmail: "daan@example.nl", totaal: 285.00, status: "te_plukken", datum: "2026-02-25T08:45:00Z" },
  ...
  { id: "14", merkId: "leather-design", ordernummer: "LD-2026-089", klantNaam: "Ruben Visser", klantEmail: "ruben@example.nl", totaal: 129.95, status: "te_plukken", datum: "2026-02-25T11:00:00Z" },
  ...
  { id: "25", merkId: "ratpack", ordernummer: "RP-2026-036", klantNaam: "Lotte Vink", klantEmail: "lotte@example.nl", totaal: 189.00, status: "te_plukken", datum: "2026-02-25T09:15:00Z" },
...
export const MOCK_BESTELLINGEN: Bestelling[] = BASE.map((b) => ({
  ...b,
  regels: buildRegels(b.merkId, b.totaal),
}));
